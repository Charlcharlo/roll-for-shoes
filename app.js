  ...
  new GoogleStrategy(
    {
      ...
      // "http://localhost:3000/auth/google/roll-for-shoes",
    },
...
mongoose.connect(
  "mongodb+srv://LineGoon:" +
    process.env.MONGO_PW +
    "@roll-for-shoes.8s4xzsv.mongodb.net/?retryWrites=true&w=majority"
);
// mongoose.connect('mongodb://127.0.0.1:25420/myapp')
...
app.post("/login", (req, res) => {
  ...
  req.login(user, (err) => {
    if (err) {
      ...
      passport.authenticate("local", { failureRedirect: "/login/retry" })(
        req,
        res,
        function () {
          res.redirect("/my-characters");
...
app.post("/:id/edit-skills", (req, res) => {
  ...
  Character.findByIdAndUpdate(id, { $set: { skills: skills } }, (err) => {
    if (!err) {
      res.redirect("/sheet/" + id);
    }
  });
...
app.post("/:id/edit-items", (req, res) => {
  ...
  Character.findByIdAndUpdate(id, { $set: { items: items } }, (err) => {
    if (!err) {
      res.redirect("/sheet/" + id);
    }
  });
...
app.post("/:id/edit-info", (req, res) => {
  const id = req.params.id;
  Character.findByIdAndUpdate(id, { $set: req.body }, (err) => {
    if (!err) {
      res.redirect("/sheet/" + id);
    } else {
      console.log(err);
